...
- **Moving between pages no longer reloads the document.** Sign-in and sign-up, and the admin console's users, OAuth clients, messages, audit log and settings, now swap the page in place, with back/forward and scroll position as you'd expect. Every URL is still rendered on the server, so a first load, a reload, a bookmark or a link opened in a new tab is the same page as before, and anything the client can't be sure of — a URL with no page, a redirect (such as the OAuth authorization hand-off), an error — is left to the browser. It uses the client-side router in `@rapidrest/react` 2.0.0 (`@rapidrest/react` is now required at `^2.0.0`). One thing to know when deploying: the page data is served as JSON from the page's own URL with `Vary: X-Rapidrest-Navigation`, so a cache or CDN in front of the server must respect `Vary`.

### Changed

- **`@rapidrest/react` 2.0.0** answers a wrong-case URL with a 404 on case-insensitive filesystems (Windows, macOS), and hides error details from the `_500` page wherever `NODE_ENV` isn't a development one — the compose files' default of `dev` is now redacted, as `production` already was. Neither changes anything for these apps' own URLs.
...
- **A policy for how many passwords an account can have** — *Allow accounts to have multiple passwords* under **Settings**, alongside *Allow new user registration*. **Off by default: one password per account.** It's configured with `auth.allowMultiplePasswords` in `config.ts` (which seeds it the first time it's read) and can then be changed in the admin console, where it's stored in the database — the same as *Allow new user registration*. With it off, nobody can add a second password to an account, so whoever can change the one it has decides who can sign in by password; with it on, an account can have several, and an administrator can keep a password of their own on it. **This is a change of behavior**: accounts could previously have any number; set `allowMultiplePasswords: true` to keep that. Turning it off later doesn't remove passwords accounts already have.
- **Admin *Set password* has two more options.** When the server allows multiple passwords and the account already has one, *Add as an additional password, keeping the user's current one* adds a password only the administrator can change, so the administrator keeps their own way into the account. Otherwise it resets the existing password, and unticking *Allow the user to change their password* now takes that right away for good — even from a password the user had chosen and could change themselves — which is how an administrator keeps control of it.
- **A *Generate* button on the New user form and on *Set password*** fills the password fields with a random password that meets the server's requirements (16 characters, or the server's minimum if longer; no look-alike characters such as `0`/`O`, `1`/`l`/`I`; from the browser's cryptographic random source). Since the admin has to read it out, the fields switch to plain text when one is generated, with *Show*/*Hide* and *Copy* buttons.

### Fixed

- **A password's "Set by administrator" label now goes away when its holder changes it**, since it's no longer true, and comes back when an administrator resets it. A label the holder chose themselves (e.g. "LastPass") is left alone.
- **Signing in with a `return_to`, as an account that must change its password, no longer loses it.** It used to be discarded. Now the account is sent to `/account` first, with the `return_to` carried along (only if it's safe), and is sent on to it once the password has been changed — never before.
- **The forced change-password dialog could fail with "Invalid object version" and leave the person stuck.** Signing in, and elevating, each stamp the password secret's `lastUsedAt` and so bump its version, so the copy the page loaded was already out of date. It now retries once with the current version. This affected *Change password* on the account page in general, not just the forced dialog.

- **A password set by an administrator could not be changed by the account holder** ("User does not have permission to perform this action."). The secret is now created with `?allowUserChange=true` when *Allow the user to change their password* is ticked.
- **Signing in failed with "Invalid authorization request" even with the right password, when this browser had cached an out-of-date account id.** The sign-in page hashes the password client-side using a salt derived from the account's uid, and caches `identifier → uid` after each successful sign-in. If that identifier later belongs to a different uid — most commonly a development database that was recreated, so `admin` gets a new uid and password on every `yarn dev` — the hash is rejected exactly like a wrong password, on every attempt. A refused hashed attempt now drops the stale cache entry and retries once with the plaintext (which the server accepts too), so a correct password signs in and repairs the cache. A wrong password is still refused, after two attempts instead of one.
- **A custom logo was squashed or stretched into a 128×128 square on the sign-in and sign-up pages.** Only the height is fixed now; the width follows the logo's own aspect ratio (and shrinks proportionally if it's wider than the page).
- **Signing in with a passkey failed** with `decodedPublicKey.get is not a function` (fixed in `@rapidrest/auth`, where the stored public key was being decoded wrongly on MongoDB).

> Both server-side fixes and `passwordChangeRequired` need a build of `@rapidrest/auth` that includes them; against an older one the password-change options in the admin form have no effect and passkey sign-in still fails.
