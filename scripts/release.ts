...
const HELM_VERSION_FILES = ["helm/*", "README.md", "single_node_install.sh"];
...
function updateHelmVersion(version: string): void {
    ...
    writeFileSync(SINGLE_NODE_INSTALL_PATH, installScript.replace(installScriptRegex, `VERSION="${version}"`));

    const readmeRegex = /\b\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?\b/g;
    const readme = readFileSync(README_PATH, "utf-8");
    writeFileSync(README_PATH, readme.replace(readmeRegex, version));
}
...
function main(): void {
    ...
    updateHelmVersion(newVersion);

    const lockFiles = ["package-lock.json", "yarn.lock"].filter((file) => existsSync(path.join(ROOT, file)));
    run("git", ["add", "package.json", ...lockFiles, "RELEASE_NOTES.md", "CHANGELOG.md", ...HELM_VERSION_FILES]);
