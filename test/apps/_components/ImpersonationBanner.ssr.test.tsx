...
// the way it is under real SSR: `localStorage` doesn't exist in Node, so `isImpersonating()`'s
// `typeof window === "undefined"` guard is what keeps `ImpersonationBanner`'s `useState(isImpersonating)`
// lazy initializer from throwing when this component is actually reached during SSR (unlike `AdminShell`'s
// pages, `AuthShell`'s children — e.g. `apps/www/account` — render for real on the server whenever the
// request already carries a valid session cookie).
