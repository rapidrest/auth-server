...
describe("UserTable", () => {
    it("renders an empty state when there are no users", () => {
        render(<UserTable users={[]} aliasesByUid={{}} onDelete={vi.fn()} onImpersonate={vi.fn()} />);
        expect(screen.getByText("No accounts found.")).toBeInTheDocument();
    });

    it("renders a row per user with aliases, roles, scopes, status, and a formatted date", () => {
        render(<UserTable users={[baseUser]} aliasesByUid={{ u1: [baseAlias] }} onDelete={vi.fn()} onImpersonate={vi.fn()} />);
    ...
    it("renders a placeholder when a user has no aliases, roles, or scopes", () => {
        render(<UserTable users={[{ ...baseUser, roles: [], scopes: [] }]} aliasesByUid={{}} onDelete={vi.fn()} onImpersonate={vi.fn()} />);
    ...
    it("tolerates a user record missing roles/scopes entirely (defensive fallback)", () => {
        ...
        render(<UserTable users={[bareUser]} aliasesByUid={{}} onDelete={vi.fn()} onImpersonate={vi.fn()} />);
        expect(screen.getAllByText("—")).toHaveLength(3);
    });
    ...
    it("falls back to the raw ISO string if toLocaleDateString throws", () => {
        ...
        render(<UserTable users={[baseUser]} aliasesByUid={{}} onDelete={vi.fn()} onImpersonate={vi.fn()} />);
        expect(screen.getByText(baseUser.dateCreated)).toBeInTheDocument();
        spy.mockRestore();
    });

    it("renders an empty date cell when dateCreated is missing", () => {
        render(<UserTable users={[{ ...baseUser, dateCreated: "" }]} aliasesByUid={{}} onDelete={vi.fn()} onImpersonate={vi.fn()} />);
    ...
    it("calls onDelete with the user when Delete is clicked", async () => {
        ...
        render(<UserTable users={[baseUser]} aliasesByUid={{}} onDelete={onDelete} onImpersonate={vi.fn()} />);
        await user.click(screen.getByRole("button", { name: "Delete" }));
        expect(onDelete).toHaveBeenCalledWith(baseUser);
    });

    it("calls onImpersonate with the user when Impersonate is clicked", async () => {
        const user = userEvent.setup();
        const onImpersonate = vi.fn();
        render(<UserTable users={[baseUser]} aliasesByUid={{}} onDelete={vi.fn()} onImpersonate={onImpersonate} />);
        await user.click(screen.getByRole("button", { name: "Impersonate" }));
        expect(onImpersonate).toHaveBeenCalledWith(baseUser);
    });
});
