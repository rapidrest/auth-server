...
// `BaseAuthMFARouteMongo`'s real `initialize()` — which requires a live `ObjectFactory`/database and is
// already exercised end-to-end by `Server.mongo.test.ts`. That integration path only ever hits the
...
import { AuthMFARoute } from "../src/mongo/routes/AuthMFARoute.js";

afterEach(() => {
    vi.restoreAllMocks();
});

describe("AuthMFARoute (mongo) — initialize()", () => {
    it("drives the registered 'mfa' strategy's require2FA from auth:require_mfa when the strategy is found", async () => {
        vi.spyOn(BaseAuthMFARouteMongo.prototype, "initialize").mockResolvedValue(undefined);
