...
import { ConnectionManager, ObjectFactory, RepoUtils, Server, isSqlDataSource } from "@rapidrest/service-core";
import { request } from "@rapidrest/service-core/test";
import * as uuid from "uuid";
import { Repository } from "typeorm";
import { SystemSettingsSQL } from "@rapidrest/auth/sql";

const SQL_DB_FILE = "rrst-test-settings-route";
const ACL_DB_FILE = "rrst-test-settings-route-acl";

describe("SettingsRoute (sql)", () => {
    ...
    it("mounts PUT at /api/settings, letting an admin change allowRegistration/requireMFA", async () => {
        ...
        const stored = await settingsRepo.findOne({ where: { uid: "default" } });
