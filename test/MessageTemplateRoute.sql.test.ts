...
// paths are covered. See `SiteSettingsRoute.sql.test.ts` for the bootstrap pattern this mirrors.
vi.mock("redis", async () => {
...
const SQL_DB_FILE = "rrst-test-message-templates";
const ACL_DB_FILE = "rrst-test-message-templates-acl";
...
describe("Message templates (sql)", () => {
    ...
    describe("seeded from the deployment's config", () => {
        ...
        it("stored the secrets encrypted, never in the clear", async () => {
            const repo = await objectFactory.newInstance(RepoUtils, { name: "MessagingSettingsSQL", args: [MessagingSettingsSQL] });
