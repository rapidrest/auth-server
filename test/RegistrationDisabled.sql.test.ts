...
///////////////////////////////////////////////////////////////////////////////
// End-to-end coverage for closing new account registration, against the real server wiring in `src/sql`: the
// `auth:allowRegistration` config seeds the stored `SystemSettings`, an admin toggles it at runtime through
// `PUT /api/settings` (the dedicated registration/MFA policy route — see `SettingsRoute.sql.test.ts` for
...
import { AliasSQL, ProfileSQL, SecretSQL, SystemSettingsSQL, UserSQL } from "@rapidrest/auth/sql";

const SQL_DB_FILE = "rrst-test-registration-disabled";
const ACL_DB_FILE = "rrst-test-registration-disabled-acl";
const PASSWORD = "S3cret!Pass123";
const OAUTH_PROVIDERS = ["google", "apple", "facebook", "microsoft"];

describe("Registration disabled (sql)", () => {
    ...
    let userRepo: RepoUtils<UserSQL>;
    let aliasRepo: RepoUtils<AliasSQL>;
    let profileRepo: RepoUtils<ProfileSQL>;
    let seeded: { stored?: boolean | null; exposed?: boolean };

    /** Provisions a password account and signs `testAgent` in as it (see `SiteSettingsRoute.sql.test.ts`). */
    async function createAndSignInUser(
        testAgent: ReturnType<typeof agent>,
        username: string,
        roles: string[] = [],
    ): Promise<string> {
        const secretRepo: RepoUtils<SecretSQL> = await objectFactory.newInstance(RepoUtils, {
            name: "SecretSQL",
            args: [SecretSQL],
        });
    ...
    beforeAll(async () => {
        ...
        userRepo = await objectFactory.newInstance(RepoUtils, { name: "UserSQL", args: [UserSQL] });
        aliasRepo = await objectFactory.newInstance(RepoUtils, { name: "AliasSQL", args: [AliasSQL] });
        profileRepo = await objectFactory.newInstance(RepoUtils, { name: "ProfileSQL", args: [ProfileSQL] });

        adminAgent = agent(server);
        // Runs before any HTTP call is made (see below): `DefaultAccountsSQL`'s own startup seeding job is
        // still writing via direct repo calls at this point, same as this helper — the very first *HTTP*
        // request is deliberately deferred until after it settles, since routing an early one through the
        // request pipeline while that job's transaction is still open triggers SQLite's "cannot start a
        // transaction within a transaction" (harmless in production, where nothing calls back into the
        // server this early, but very reachable here).
        ...
        const systemSettingsRepo: RepoUtils<SystemSettingsSQL> = await objectFactory.newInstance(RepoUtils, {
            name: SystemSettingsSQL.name,
            args: [SystemSettingsSQL],
        });
